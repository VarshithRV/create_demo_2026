...
public:
  ...
  {
    // --- Callback group (reentrant so we can block and still get client responses) ---
    ...
    human_to_robot_handover_ = this->create_service<std_srvs::srv::Trigger>(
      ...
      rmw_qos_profile_services_default,
      callback_group_);  // same reentrant group
    ...
    object_pose_subscription_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      ...
      [this](const geometry_msgs::msg::PoseStamped::SharedPtr msg)
      {
        ...
        // Linear + angular offset in camera frame
        Eigen::Vector3d p_in(
...
private:
    ...
    const std::string &name)
  {
    ...
    // Because we are in a Reentrant callback group and using a MultiThreadedExecutor,
    // another thread can execute the response callback while we wait here.
  ...
  void handover_callback_(const std_srvs::srv::Trigger::Request::SharedPtr /*request*/,
